...
import com.remindme.tracker.R
import com.remindme.tracker.model.Tracker
import com.remindme.tracker.presentation.components.TaskGraph
import com.remindme.tracker.presentation.components.TaskTrackerList
...
@Composable
internal fun TrackerLoader(viewModel: TrackerViewModel = hiltViewModel(), onUpPress: () -> Unit) {
    val data by remember { viewModel.loadTracker() }
        .collectAsState(initial = TrackerViewState.Loading)

    Scaffold(topBar = { RemindMeToolbar(onUpPress = onUpPress) }) {
        Crossfade(data) { state ->
            when (state) {
                ...
                TrackerViewState.Loading -> RemindMeLoadingContent()
            }
        }
    }
}
